...
for i in range(num_interpolations):
    ...
    if x_user_value <= (x_values[0]):
        ...
        output = ((y_values[1]-y_values[0])/(x_values[1]-x_values[0]))*(x_user_value-x_values[0])+y_values[0]
    ...
    # checks to see if the x value entered is greater than all the x values inputted
        ...
        # uses the interpolation formula into variable output
        output = ((y_values[len(y_values)-1]-y_values[len(y_values)-2])/(x_values[len(x_values)-1]-x_values[len(x_values)-2]))*\
